...
if page == "🏠  Dashboard":
    ...
    if not assistant_id:
        ...
        # Write the widget HTML to the local HTTP server.
        # This gives the iframe a REAL origin (http://127.0.0.1:PORT)
        # so Daily.co postMessage works without null-origin errors.
        _vk  = VAPI_PUBLIC_KEY
        _aid = assistant_id
        ...
        widget_html = (
            ...
            "html,body{background:#020B18;color:#E2E8F0;"
            "font-family:-apple-system,BlinkMacSystemFont,'Inter',sans-serif;padding:14px;}"
            ".card{background:linear-gradient(145deg,#071220,#050E1A);"
            "border:1px solid #0A1E38;border-radius:18px;padding:20px;"
            "box-shadow:0 20px 60px rgba(0,0,0,0.6);}"
            ".top{display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;}"
            ".brand-title{font-size:0.95rem;font-weight:800;"
            "background:linear-gradient(135deg,#60A5FA,#A78BFA);"
            "-webkit-background-clip:text;-webkit-text-fill-color:transparent;}"
            ".brand-sub{font-size:0.62rem;color:#1E3A5F;margin-top:2px;}"
            ".chip{display:flex;align-items:center;gap:6px;background:#030A16;"
            "padding:5px 12px;border-radius:999px;border:1px solid #0A1E38;"
            "font-size:0.65rem;color:#1E3A5F;font-weight:600;}"
            ".dot{width:7px;height:7px;border-radius:50%;background:#1E3A5F;"
            "transition:all 0.4s;flex-shrink:0;}"
            ".banner{display:flex;align-items:flex-start;gap:10px;"
            "background:rgba(251,191,36,0.07);border:1px solid rgba(251,191,36,0.2);"
            "border-radius:10px;padding:10px 13px;margin-bottom:13px;"
            "font-size:0.76rem;color:#FDE68A;line-height:1.5;}"
            ".retry{display:none;align-items:center;justify-content:space-between;gap:10px;"
            "background:rgba(96,165,250,0.07);border:1px solid rgba(96,165,250,0.2);"
            "border-radius:10px;padding:10px 13px;margin-bottom:13px;"
            "font-size:0.76rem;color:#93C5FD;}"
            ".retry.show{display:flex;}"
            ".rbtn{background:#2563EB;color:white;border:none;border-radius:7px;"
            "padding:5px 12px;font-size:0.72rem;font-weight:700;cursor:pointer;flex-shrink:0;}"
            ".btns{display:flex;gap:10px;margin-bottom:13px;}"
            ".btn{flex:1;border:none;padding:13px;border-radius:11px;"
            ...
            ".bstart{background:linear-gradient(135deg,#2563EB,#4F46E5);"
            "box-shadow:0 6px 20px rgba(37,99,235,0.3);}"
            ".bstop{background:linear-gradient(135deg,#991B1B,#7F1D1D);}"
            ".sbox{background:#030A16;border:1px solid #0A1E38;border-radius:10px;"
            "padding:12px 14px;margin-bottom:12px;}"
            "#sw{font-size:0.86rem;font-weight:700;color:#34D399;}"
            ".grid{display:grid;grid-template-columns:repeat(4,1fr);gap:7px;margin-bottom:12px;}"
            ".mc{background:#030A16;border:1px solid #0A1E38;border-radius:10px;padding:10px;}"
            ".ml{color:#1E3A5F;font-size:0.54rem;text-transform:uppercase;"
            "letter-spacing:0.1em;margin-bottom:3px;font-weight:800;}"
            ".mv{font-size:0.8rem;font-weight:800;color:#E2E8F0;}"
            ".slbl{color:#1E3A5F;font-size:0.6rem;font-weight:700;"
            "text-transform:uppercase;letter-spacing:0.09em;margin-bottom:5px;}"
            "canvas#wv{width:100%;height:40px;border-radius:8px;"
            "background:#030A16;border:1px solid #0A1E38;display:block;margin-bottom:12px;}"
            ".tlbl{display:flex;justify-content:space-between;}"
            "#tx{background:#020810;border:1px solid #0A1E38;border-radius:10px;"
            "padding:12px;height:190px;overflow-y:auto;margin-bottom:12px;}"
            ".te{display:flex;align-items:center;justify-content:center;"
            "height:100%;color:#1E3A5F;font-size:0.74rem;text-align:center;}"
            ".turn{display:flex;gap:8px;align-items:flex-start;"
            "padding:8px 0;border-bottom:1px solid #030A16;}"
            ".turn:last-child{border-bottom:none;}"
            ".av{width:22px;height:22px;border-radius:6px;display:flex;"
            "align-items:center;justify-content:center;"
            "font-size:0.65rem;font-weight:800;flex-shrink:0;}"
            ".av.ai{background:rgba(167,139,250,0.15);color:#A78BFA;}"
            ".av.usr{background:rgba(52,211,153,0.15);color:#34D399;}"
            ".tb{flex:1;min-width:0;}"
            ".th{display:flex;align-items:center;gap:6px;margin-bottom:2px;}"
            ".tn{font-size:0.68rem;font-weight:700;color:#64748B;}"
            ".stag{font-size:0.56rem;font-weight:700;padding:1px 6px;"
            "border-radius:999px;text-transform:uppercase;}"
            ".tt{font-size:0.74rem;line-height:1.5;}"
            "#lg{background:#020810;border:1px solid #0A1E38;border-radius:10px;"
            "padding:10px;height:100px;overflow-y:auto;}"
            ".lr{display:flex;gap:7px;padding:4px 0;"
            "border-bottom:1px solid #030A16;font-size:0.68rem;}"
            ".lt{color:#0A1E38;font-size:0.6rem;flex-shrink:0;font-family:monospace;}"
            ...
            "<div class='top'>"
            "<div>"
            "<div class='brand-title'>HireAI Live Interview</div>"
            "<div class='brand-sub'>Enterprise Voice Platform · Vapi AI</div>"
            "</div>"
            "<div class='chip'><div class='dot' id='dot'></div><span id='chip'>READY</span></div>"
            "</div>"
            "<div class='banner' id='mic-banner'>"
            "<span>🎤</span>"
            "<span>Click <strong>Start Interview</strong> and allow microphone access when asked. "
            "If blocked, click the lock icon in the address bar &rarr; Microphone &rarr; Allow &rarr; refresh.</span>"
            "</div>"
            "<div class='retry' id='retry'>"
            "<span id='retry-msg'>Call dropped. Click Resume to continue.</span>"
            ...
            "<div class='mc'><div class='ml'>Connection</div>"
            "<div class='mv' id='conn' style='font-size:0.72rem;'>IDLE</div></div>"
            "<div class='mc'><div class='ml'>Microphone</div>"
            "<div class='mv' id='aud' style='font-size:0.72rem;'>STANDBY</div></div>"
            "<div class='mc'><div class='ml'>AI Status</div>"
            "<div class='mv' id='ai' style='font-size:0.72rem;'>&#x2014;</div></div>"
            "</div>"
            "<div class='slbl'>Audio Waveform</div>"
            "<canvas id='wv'></canvas>"
            "<div class='slbl tlbl'>"
            "<span>Live Transcript</span><span id='tc'>0 turns</span>"
            "</div>"
            "<div id='tx'>"
            "<div class='te' id='te'>The conversation will appear here as it happens.</div>"
            "</div>"
        ...
        # JavaScript — classic script, dynamic import for Vapi SDK
        # addEventListener (not onclick) so buttons work in iframes
        js = (
            "<script type='module'>"
            f"const VK='{_vk}',AID='{_aid}';"
            ...
            "const ts=document.createElement('span');ts.className='lt';"
            "ts.textContent=new Date().toLocaleTimeString();"
            "const tx=document.createElement('span');"
            "tx.style.color=LC[t]||LC.info;tx.textContent=m;"
            ...
            "tmr=setInterval(()=>{if(!active)return;"
            "const s=Math.floor((Date.now()-t0)/1000);"
            "$('timer').textContent=Math.floor(s/60)+':'+String(s%60).padStart(2,'0');},1000);}"
            ...
            "function rsz(){cv.width=cv.offsetWidth*devicePixelRatio;"
            ...
            "function senti(t){const tx=' '+t.toLowerCase()+' ';"
            "const h=HES.filter(m=>tx.includes(m)).length;"
            "const c=CON.filter(m=>tx.includes(m)).length;"
            ...
            "const pan=$('tx');"
            "if(!fin&&lPEl&&lPRole===role){lPEl.querySelector('.tt').textContent=text;"
            "pan.scrollTop=pan.scrollHeight;return;}"
            "const w=document.createElement('div');w.className='turn';"
            "const isAI=role==='assistant';"
            "const av=document.createElement('div');av.className='av '+(isAI?'ai':'usr');"
            ...
            "nm.textContent=isAI?'Alex':'Candidate';hd.appendChild(nm);"
            "if(!isAI&&fin){const s=senti(text),tg=document.createElement('span');"
            "tg.className='stag';tg.style.background=SB[s];tg.style.color=SC[s];"
            "tg.textContent=s;hd.appendChild(tg);}"
            "const tt=document.createElement('div');tt.className='tt';"
            "tt.style.color=fin?'#CBD5E1':'#334155';"
            "tt.style.fontStyle=fin?'normal':'italic';"
            "tt.textContent=text;bd.append(hd,tt);w.append(av,bd);"
            "pan.appendChild(w);pan.scrollTop=pan.scrollHeight;"
            "if(!fin){lPEl=w;lPRole=role;}"
            "else{lPEl=null;lPRole=null;turns++;$('tc').textContent=turns+(turns===1?' turn':' turns');}}"
            "function wire(){"
            "if(reg)return;reg=true;"
            "vapi.on('call-start',()=>{"
            "active=true;drop=false;$('retry').classList.remove('show');startTmr();"
            "st('Interview is live — Alex is speaking first','#34D399');"
            "$('dot').style.background='#34D399';$('dot').style.boxShadow='0 0 10px #34D399';"
            "$('chip').textContent='LIVE';$('conn').textContent='CONNECTED';"
            "$('ai').textContent='ACTIVE';log('Interview started','success');});"
            "vapi.on('speech-start',()=>{st('Alex is speaking — listen carefully','#A78BFA');"
            "$('aud').textContent='AI SPEAKING';$('ai').textContent='SPEAKING';});"
            "vapi.on('speech-end',()=>{st('Your turn — speak clearly','#34D399');"
            "$('aud').textContent='LISTENING';$('ai').textContent='LISTENING';});"
            "vapi.on('volume-level',v=>{vol=v;});"
            "vapi.on('message',msg=>{if(msg?.type==='transcript'&&msg?.transcript)"
            "addTurn(msg.role,msg.transcript,msg.transcriptType==='final');});"
            "vapi.on('error',e=>{"
            "const m=e?.message||e?.errorMsg||JSON.stringify(e);"
            "log('Error: '+m,'error');st('Connection error — see log','#EF4444');"
            "$('conn').textContent='ERROR';if(active&&!clean)drop=true;});"
            "vapi.on('call-end',async()=>{"
            "if(active&&!clean&&drop){"
            "log('Call dropped — click Resume','warning');"
            "st('Connection dropped. Click Resume to continue.','#FBB724');"
            "$('retry').classList.add('show');}"
            "else{log('Interview complete — scores processing','warning');"
            "st('Done! Scores appear in Candidates within 60 seconds.','#FBB724');}"
            ...
            "$('mic-banner').style.display='none';"
            "$('retry').classList.remove('show');"
            "try{"
            "log('Requesting microphone...','system');"
            "st('Requesting microphone — allow when prompted','#F59E0B');"
            ...
            "throw new Error('Microphone denied. Click lock icon → Microphone → Allow → refresh.');"
            ...
            "throw me;}"
            "log('Microphone granted','success');$('aud').textContent='MIC READY';"
            "stream.getTracks().forEach(t=>t.stop());"
            "log('Loading Vapi SDK...','system');st('Loading Vapi SDK...','#3B82F6');"
            "const {default:Vapi}=await import('https://esm.sh/@vapi-ai/web@2.3.2');"
            "vapi=new Vapi(VK);window._vapi=vapi;"
            "log('Vapi SDK ready','success');"
            "wire();"
            "log('Connecting to assistant...','system');"
            "st('Connecting...','#3B82F6');$('conn').textContent='CONNECTING';"
            "await vapi.start(AID);"
            "log('Connected — interview starting','success');}"
            "catch(err){"
            "const m=err?.message||String(err);"
            "log('Error: '+m,'error');st(m,'#EF4444');"
            "$('mic-banner').style.display='flex';await cleanup();}"
            "finally{init=false;}}"
            "async function hwStop(){"
            "clean=true;drop=false;"
            "st('Ending session...','#F59E0B');log('Ending interview','warning');"
            "if(window._vapi)await window._vapi.stop();"
            "await cleanup();"
            "st('Interview ended. Check Candidates for scores.','#475569');"
            "log('Session ended','success');}"
            "async function hwResume(){"
            "$('retry').classList.remove('show');"
            "log('Resuming...','system');await hwStart();}"
        ...
        full_html = widget_html + js

        # Write to local HTTP server and serve via iframe
        ...
        <div style="background:rgba(37,99,235,0.07);border-left:4px solid #3B82F6;
            ...
            <br><br>
            <strong style="color:#FBB724;">On Streamlit Cloud?</strong>
            The interview widget requires a local server to work around browser security restrictions.
            For cloud deployment, run the backend + frontend locally and use ngrok to share the URL.
        </div>""", unsafe_allow_html=True)
