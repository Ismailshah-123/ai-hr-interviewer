...
if page == "🏠  Dashboard":
    ...
    if not assistant_id:
        ...
        # Build widget as a complete HTML page string
        # Uses Vapi SDK v2.3.2 which uses Vapi's OWN WebRTC transport
        # — does NOT use Daily.co at all — no null-origin postMessage error
        # Rendered via components.html() which works on Streamlit Cloud
...
body{{background:#020B18;color:#E2E8F0;font-family:-apple-system,BlinkMacSystemFont,'Inter',sans-serif;padding:12px;}}
.card{{background:linear-gradient(145deg,#071220,#050E1A);border:1px solid #0A1E38;border-radius:16px;padding:18px;}}
.top{{display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;}}
.title{{font-size:0.9rem;font-weight:800;background:linear-gradient(135deg,#60A5FA,#A78BFA);-webkit-background-clip:text;-webkit-text-fill-color:transparent;}}
.sub{{font-size:0.6rem;color:#1E3A5F;margin-top:2px;}}
.chip{{display:flex;align-items:center;gap:6px;background:#030A16;padding:5px 11px;border-radius:999px;border:1px solid #0A1E38;font-size:0.63rem;color:#1E3A5F;font-weight:600;}}
.dot{{width:7px;height:7px;border-radius:50%;background:#1E3A5F;transition:all 0.4s;flex-shrink:0;}}
.banner{{display:flex;align-items:flex-start;gap:9px;background:rgba(251,191,36,0.07);border:1px solid rgba(251,191,36,0.2);border-radius:9px;padding:9px 12px;margin-bottom:12px;font-size:0.74rem;color:#FDE68A;line-height:1.5;}}
.retry{{display:none;align-items:center;justify-content:space-between;gap:10px;background:rgba(96,165,250,0.07);border:1px solid rgba(96,165,250,0.2);border-radius:9px;padding:9px 12px;margin-bottom:12px;font-size:0.74rem;color:#93C5FD;}}
.retry.show{{display:flex;}}
.rbtn{{background:#2563EB;color:white;border:none;border-radius:7px;padding:5px 11px;font-size:0.7rem;font-weight:700;cursor:pointer;flex-shrink:0;}}
.btns{{display:flex;gap:9px;margin-bottom:12px;}}
.btn{{flex:1;border:none;padding:12px;border-radius:10px;color:white;font-size:0.84rem;font-weight:800;cursor:pointer;transition:all 0.2s;font-family:inherit;}}
.btn:hover:not(:disabled){{transform:translateY(-2px);opacity:0.9;}}
.btn:disabled{{opacity:0.35;cursor:not-allowed;transform:none;}}
.bstart{{background:linear-gradient(135deg,#2563EB,#4F46E5);box-shadow:0 6px 20px rgba(37,99,235,0.3);}}
.bstop{{background:linear-gradient(135deg,#991B1B,#7F1D1D);}}
.sbox{{background:#030A16;border:1px solid #0A1E38;border-radius:9px;padding:11px 13px;margin-bottom:11px;}}
#sw{{font-size:0.84rem;font-weight:700;color:#34D399;}}
.grid{{display:grid;grid-template-columns:repeat(4,1fr);gap:7px;margin-bottom:11px;}}
.mc{{background:#030A16;border:1px solid #0A1E38;border-radius:9px;padding:9px;}}
.ml{{color:#1E3A5F;font-size:0.52rem;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:3px;font-weight:800;}}
.mv{{font-size:0.78rem;font-weight:800;color:#E2E8F0;}}
.lbl{{color:#1E3A5F;font-size:0.58rem;font-weight:700;text-transform:uppercase;letter-spacing:0.09em;margin-bottom:4px;}}
canvas{{width:100%;height:38px;border-radius:7px;background:#030A16;border:1px solid #0A1E38;display:block;margin-bottom:11px;}}
.tlbl{{display:flex;justify-content:space-between;}}
#tx{{background:#020810;border:1px solid #0A1E38;border-radius:9px;padding:11px;height:180px;overflow-y:auto;margin-bottom:11px;}}
.te{{display:flex;align-items:center;justify-content:center;height:100%;color:#1E3A5F;font-size:0.72rem;text-align:center;}}
.turn{{display:flex;gap:7px;align-items:flex-start;padding:7px 0;border-bottom:1px solid #030A16;}}
.turn:last-child{{border-bottom:none;}}
.av{{width:20px;height:20px;border-radius:6px;display:flex;align-items:center;justify-content:center;font-size:0.62rem;font-weight:800;flex-shrink:0;}}
.ai{{background:rgba(167,139,250,0.15);color:#A78BFA;}}
.usr{{background:rgba(52,211,153,0.15);color:#34D399;}}
.tb{{flex:1;min-width:0;}}
.th{{display:flex;align-items:center;gap:5px;margin-bottom:2px;}}
.tn{{font-size:0.66rem;font-weight:700;color:#64748B;}}
.stag{{font-size:0.54rem;font-weight:700;padding:1px 5px;border-radius:999px;text-transform:uppercase;}}
.tt{{font-size:0.72rem;line-height:1.5;}}
#lg{{background:#020810;border:1px solid #0A1E38;border-radius:9px;padding:9px;height:90px;overflow-y:auto;}}
.lr{{display:flex;gap:6px;padding:3px 0;border-bottom:1px solid #030A16;font-size:0.66rem;}}
.lt{{color:#0A1E38;font-size:0.58rem;flex-shrink:0;font-family:monospace;}}
::-webkit-scrollbar{{width:3px;}}
::-webkit-scrollbar-thumb{{background:#0A1E38;border-radius:2px;}}
</style>
</head>
<body>
<div class="card">
  <div class="top">
    <div><div class="title">HireAI Live Interview</div><div class="sub">Enterprise Voice Platform · Vapi AI</div></div>
    <div class="chip"><div class="dot" id="dot"></div><span id="chip">READY</span></div>
  </div>
  <div class="banner" id="mic-banner">
    <span>🎤</span>
    <span>Click <strong>Start Interview</strong> then allow microphone when your browser asks. If blocked, click the lock icon in the address bar &rarr; Microphone &rarr; Allow &rarr; refresh.</span>
  </div>
  <div class="retry" id="retry">
    <span>Call dropped. Click Resume to reconnect and continue.</span>
    <button class="rbtn" id="btn-resume">Resume</button>
  </div>
  <div class="btns">
    <button class="btn bstart" id="btn-start">&#128640; Start Interview</button>
    <button class="btn bstop" id="btn-stop" disabled>&#128721; End Interview</button>
  </div>
  <div class="sbox"><div id="sw">System ready &mdash; click Start Interview to begin.</div></div>
  <div class="grid">
    <div class="mc"><div class="ml">Timer</div><div class="mv" id="timer">0:00</div></div>
    <div class="mc"><div class="ml">Connection</div><div class="mv" id="conn" style="font-size:0.7rem;">IDLE</div></div>
    <div class="mc"><div class="ml">Microphone</div><div class="mv" id="aud" style="font-size:0.7rem;">STANDBY</div></div>
    <div class="mc"><div class="ml">AI Status</div><div class="mv" id="ai" style="font-size:0.7rem;">&mdash;</div></div>
  </div>
  <div class="lbl">Audio Waveform</div>
  <canvas id="wv"></canvas>
  <div class="lbl tlbl"><span>Live Transcript</span><span id="tc">0 turns</span></div>
  <div id="tx"><div class="te" id="te">The conversation will appear here as it happens.</div></div>
...
const VK='{_vk}',AID='{_aid}';
let vapi=null,active=false,init=false,t0=null,tmr=null,reg=false;
let vol=0,turns=0,lPEl=null,lPRole=null,clean=false,drop=false;
const $=id=>document.getElementById(id);

const LC={{info:'#334155',error:'#F87171',success:'#34D399',warning:'#FBB724',system:'#A78BFA'}};
function log(m,t='info'){{
  const r=document.createElement('div');r.className='lr';
  const ts=document.createElement('span');ts.className='lt';ts.textContent=new Date().toLocaleTimeString();
  const tx=document.createElement('span');tx.style.color=LC[t]||LC.info;tx.textContent=m;
  r.append(ts,tx);$('lg').append(r);$('lg').scrollTop=$('lg').scrollHeight;
}}
function st(m,c='#34D399'){{$('sw').textContent=m;$('sw').style.color=c;}}
function setBtns(on){{
  $('btn-start').disabled=on;$('btn-start').style.opacity=on?'0.3':'1';
  $('btn-stop').disabled=!on;$('btn-stop').style.opacity=on?'1':'0.3';
}}
function startTmr(){{
  t0=Date.now();clearInterval(tmr);
  tmr=setInterval(()=>{{if(!active)return;const s=Math.floor((Date.now()-t0)/1000);$('timer').textContent=Math.floor(s/60)+':'+String(s%60).padStart(2,'0');}},1000);
}}
async function cleanup(){{
  clearInterval(tmr);vol=0;
  $('dot').style.background='#1E3A5F';$('dot').style.boxShadow='';
  $('chip').textContent='READY';$('conn').textContent='IDLE';
  $('aud').textContent='STANDBY';$('ai').textContent='—';
  setBtns(false);active=false;init=false;
  if(vapi){{try{{await vapi.stop();}}catch(_){{}}}}vapi=null;
}}
const cv=$('wv'),ctx=cv.getContext('2d');
function rsz(){{cv.width=cv.offsetWidth*devicePixelRatio;cv.height=cv.offsetHeight*devicePixelRatio;ctx.scale(devicePixelRatio,devicePixelRatio);}}
...
function dw(){{
  ...
  const bw=W/48,gap=2;
  bars.forEach((v,i)=>{{
    const h=Math.max(2,v*H*0.85),x=i*bw+gap/2,a=active?0.9:0.25;
    const rgb=v>0.6?'248,113,113':v>0.3?'96,165,250':'52,211,153';
    ctx.fillStyle=active?`rgba(${{rgb}},${{a}})`:'rgba(30,58,95,0.5)';
    ctx.beginPath();ctx.roundRect(x,(H-h)/2,bw-gap,h,2);ctx.fill();
  }});
  bars.shift();bars.push(active?Math.min(1,vol+Math.random()*0.06):Math.random()*0.04);
  requestAnimationFrame(dw);
}}
dw();
const HES=[' um ',' uh ','i think','maybe','not sure','i guess','sort of','kind of','probably','i suppose','to be honest'];
const CON=['i built','i led','i designed','i implemented','i deployed','i created','specifically','exactly','absolutely','definitely','we shipped','production'];
function senti(t){{const tx=' '+t.toLowerCase()+' ';const h=HES.filter(m=>tx.includes(m)).length;const c=CON.filter(m=>tx.includes(m)).length;return c>h&&c>0?'confident':h>c&&h>0?'hesitant':'neutral';}}
const SC={{confident:'#34D399',neutral:'#64748B',hesitant:'#FBB724'}};
const SB={{confident:'rgba(52,211,153,0.1)',neutral:'rgba(100,116,139,0.1)',hesitant:'rgba(251,191,36,0.1)'}};
function addTurn(role,text,fin){{
  const em=$('te');if(em)em.remove();
  const pan=$('tx');
  if(!fin&&lPEl&&lPRole===role){{lPEl.querySelector('.tt').textContent=text;pan.scrollTop=pan.scrollHeight;return;}}
  const w=document.createElement('div');w.className='turn';
  const isAI=role==='assistant';
  const av=document.createElement('div');av.className='av '+(isAI?'ai':'usr');av.textContent=isAI?'AI':'C';
  const bd=document.createElement('div');bd.className='tb';
  const hd=document.createElement('div');hd.className='th';
  const nm=document.createElement('span');nm.className='tn';nm.textContent=isAI?'Alex':'Candidate';hd.appendChild(nm);
  if(!isAI&&fin){{const s=senti(text),tg=document.createElement('span');tg.className='stag';tg.style.background=SB[s];tg.style.color=SC[s];tg.textContent=s;hd.appendChild(tg);}}
  const tt=document.createElement('div');tt.className='tt';tt.style.color=fin?'#CBD5E1':'#334155';tt.style.fontStyle=fin?'normal':'italic';tt.textContent=text;
  bd.append(hd,tt);w.append(av,bd);pan.appendChild(w);pan.scrollTop=pan.scrollHeight;
  if(!fin){{lPEl=w;lPRole=role;}}else{{lPEl=null;lPRole=null;turns++;$('tc').textContent=turns+(turns===1?' turn':' turns');}}
}}
function wire(){{
  if(reg)return;reg=true;
  vapi.on('call-start',()=>{{active=true;drop=false;$('retry').classList.remove('show');startTmr();st('Interview is live — Alex is speaking first','#34D399');$('dot').style.background='#34D399';$('dot').style.boxShadow='0 0 10px #34D399';$('chip').textContent='LIVE';$('conn').textContent='CONNECTED';$('ai').textContent='ACTIVE';log('Interview started','success');}});
  vapi.on('speech-start',()=>{{st('Alex is speaking — listen carefully','#A78BFA');$('aud').textContent='AI SPEAKING';$('ai').textContent='SPEAKING';}});
  vapi.on('speech-end',()=>{{st('Your turn — speak clearly','#34D399');$('aud').textContent='LISTENING';$('ai').textContent='LISTENING';}});
  vapi.on('volume-level',v=>{{vol=v;}});
  vapi.on('message',msg=>{{if(msg?.type==='transcript'&&msg?.transcript)addTurn(msg.role,msg.transcript,msg.transcriptType==='final');}});
  vapi.on('error',e=>{{const m=e?.message||e?.errorMsg||JSON.stringify(e);log('Error: '+m,'error');st('Error — see log below','#EF4444');$('conn').textContent='ERROR';if(active&&!clean)drop=true;}});
  vapi.on('call-end',async()=>{{
    if(active&&!clean&&drop){{log('Call dropped — click Resume','warning');st('Connection dropped. Click Resume.','#FBB724');$('retry').classList.add('show');}}
    else{{log('Interview complete — scores processing','warning');st('Done! Scores appear in Candidates within 60 seconds.','#FBB724');}}
    $('chip').textContent='ENDED';await cleanup();
  }});
}}
async function hwStart(){{
  if(init||active){{log('Session already active','warning');return;}}
  init=true;setBtns(true);clean=false;
  $('mic-banner').style.display='none';$('retry').classList.remove('show');
  try{{
    log('Requesting microphone...','system');st('Requesting microphone — allow when prompted','#F59E0B');
    if(!navigator.mediaDevices?.getUserMedia)throw new Error('Microphone API unavailable. This page needs HTTPS.');
    let stream;
    try{{stream=await navigator.mediaDevices.getUserMedia({{audio:{{echoCancellation:true,noiseSuppression:true,sampleRate:16000}}}});}}
    catch(me){{
      const n=me.name;
      if(n==='NotAllowedError'||n==='PermissionDeniedError')throw new Error('Microphone denied. Click lock icon → Microphone → Allow → refresh.');
      if(n==='NotFoundError')throw new Error('No microphone found. Plug in a headset.');
      if(n==='NotReadableError')throw new Error('Mic in use by another app. Close other tabs.');
      throw me;
    }}
    log('Microphone granted','success');$('aud').textContent='MIC READY';stream.getTracks().forEach(t=>t.stop());
    log('Initialising Vapi...','system');st('Initialising Vapi...','#3B82F6');
    vapi=new Vapi(VK);window._vapi=vapi;
    wire();
    log('Connecting to assistant...','system');st('Connecting...','#3B82F6');$('conn').textContent='CONNECTING';
    await vapi.start(AID);
    log('Connected — interview starting','success');
  }}catch(err){{
    const m=err?.message||String(err);log('Error: '+m,'error');st(m,'#EF4444');
    $('mic-banner').style.display='flex';await cleanup();
  }}finally{{init=false;}}
}}
async function hwStop(){{
  clean=true;drop=false;st('Ending...','#F59E0B');log('Ending interview','warning');
  if(window._vapi)await window._vapi.stop();
  await cleanup();st('Interview ended. Check Candidates for scores.','#475569');log('Session ended','success');
}}
async function hwResume(){{$('retry').classList.remove('show');log('Resuming...','system');await hwStart();}}
$('btn-start').addEventListener('click',hwStart);
$('btn-stop').addEventListener('click',hwStop);
$('btn-resume').addEventListener('click',hwResume);
window.addEventListener('beforeunload',async()=>{{await cleanup();}});
log('HireAI interview platform ready','success');
</script>
</body></html>"""

        # ── Serve widget from Render backend (real HTTPS origin) ──
        # components.html() uses srcdoc → null origin → Daily.co fails.
        # Serving from BACKEND_URL gives a real HTTPS parent origin
        # so Daily.co postMessage works on both local and Streamlit Cloud.

        # Upload widget HTML to backend, then iframe it
        try:
            _up = requests.post(
                f"{BACKEND_URL}/vapi-widget",
                json={"html": widget, "assistant_id": _aid},
                timeout=10
            )
            if _up.status_code == 200:
                _widget_url = f"{BACKEND_URL}/vapi-widget/{_aid}"
                components.iframe(_widget_url, height=960, scrolling=False)
            else:
                # Fallback: use local server if backend upload fails
                _fname = "vapi_" + hashlib.md5(_aid.encode()).hexdigest()[:12] + ".html"
                _fpath = os.path.join(WIDGET_DIR, _fname)
                with open(_fpath, "w", encoding="utf-8") as _f:
                    _f.write(widget)
                components.iframe(f"http://127.0.0.1:{WIDGET_PORT}/{_fname}", height=960, scrolling=False)
        except Exception:
            # Final fallback: local server
            _fname = "vapi_" + hashlib.md5(_aid.encode()).hexdigest()[:12] + ".html"
            _fpath = os.path.join(WIDGET_DIR, _fname)
            with open(_fpath, "w", encoding="utf-8") as _f:
                _f.write(widget)
        ...
        <div style="background:rgba(37,99,235,0.07);border-left:4px solid #3B82F6;
            ...
            Scores appear in the Candidates page within 60 seconds of the call finishing.
        </div>""", unsafe_allow_html=True)
