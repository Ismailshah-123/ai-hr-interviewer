...
log("HireAI interview platform ready","success");
        ...
        # Render the widget directly through Streamlit's own iframe.
        # IMPORTANT: a data:text/html base64 iframe has origin "null",
        # and Daily.co's call-machine-object-bundle.js calls postMessage
        # against that origin, which Chrome/Firefox reject outright —
        # that produced the "Invalid target origin 'null'" crash.
        # components.html() uses Streamlit's own srcdoc iframe, which
        # carries a resolvable parent origin, so Daily.co's postMessage
        # succeeds and the call connects normally.
        # Write widget to local HTTP server and serve via iframe.
        # This gives the iframe a real origin (http://127.0.0.1:PORT)
        # so Daily.co's postMessage calls succeed.
